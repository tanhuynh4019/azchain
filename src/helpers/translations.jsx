const translation = {
    'About us': 'Chúng tôi',
    Token: 'Token',
    Ecosystem: 'Hệ sinh thái',
    Roadmap: 'Lộ trình',
    Contract: 'Hợp đồng',
    'Multi-chain - Multi-services': 'Đa Chain - Đa Dịch Vụ',
    'Secure DeFi Infrastructure': 'Cơ sở hạ tầng DeFi an toàn',
    'JOIN NOW': 'THAM GIA NGAY',
    'WHITE PAGE': 'TRANG TRẮNG',
    About: 'Giới thiệu về',
    'In the wave of global economic transformation towards digital solutions, coupled with the trend of green and sustainable economic development, AZChain emerges as a specialized blockchain platform, pioneering in meeting innovation needs.':
        'Trước làn sóng chuyển đổi mạnh mẽ của nền kinh tế toàn cầu sang các giải pháp số, cùng với xu hướng phát triển kinh tế xanh và bền vững, AZChain ra đời, trở thành một nền tảng blockchain chuyên biệt, tiên phong trong việc đáp ứng những nhu cầu đổi mới.',
    'AZChain provides secure and transparent blockchain solutions for data management and asset tokenization, with the ability to process transactions quickly and with high security. It addresses diverse needs in finance, commerce, and governance, while also contributing to the advancement of the digital and green economy.':
        'AZChain mang đến những giải pháp Blockchain an toàn, minh bạch trong việc quản lý dữ liệu và mã hóa tài sản thực, với khả năng thực hiện giao dịch nhanh chóng và bảo mật cao. Nhằm đáp ứng nhu cầu đa dạng trong lĩnh vực tài chính, thương mại và quản trị, cũng như góp phần thúc đẩy cả nền kinh tế số và nền kinh tế xanh.',
    'High security and safety': 'An toàn và bảo mật cao',
    'AZChain provides data encryption mechanisms and two-factor authentication to ensure your assets are safely protected from risks of loss or unauthorized access.':
        'AZChain cung cấp cơ chế mã hóa dữ liệu và xác thực hai yếu tố để đảm bảo tài sản của bạn được bảo vệ an toàn khỏi nguy cơ mất mát hoặc truy cập trái phép.',
    'Easy to use': 'Dễ sử dụng',
    'AZChain offers a simple and user-friendly interface, allowing users to easily manage and trade cryptocurrencies conveniently and quickly.':
        'AZChain cung cấp giao diện đơn giản và thân thiện với người dùng, cho phép người dùng dễ dàng quản lý và giao dịch tiền điện tử một cách thuận tiện và nhanh chóng.',
    'Versatile and convenient': 'Đa năng và tiện lợi',
    'AZChain allows users to store and manage multiple cryptocurrencies across various networks on a single platform.':
        'AZChain cho phép người dùng lưu trữ và quản lý nhiều loại tiền điện tử trên nhiều mạng lưới khác nhau trên một nền tảng duy nhất.',
    'Secure gateway to the ecosystem': 'CỔNG AN TOÀN ĐỂ DẪN VÀO HỆ SINH THÁI',
    'As part of our commitment to empower users with seamless access to the AZChain ecosystem, we are proud to introduce AZGPay Wallet, an advanced multi-chain and multi-payment method digital wallet designed with security and usability at its core.':
        'Là một phần trong cam kết của chúng tôi nhằm trao quyền cho người dùng quyền truy cập liền mạch vào hệ sinh thái AZChain, chúng tôi tự hào giới thiệu Ví AZGPay, một loại ví kỹ thuật số tiên tiến đa chain và đa phương thức thanh toán được thiết kế với cốt lõi là tính bảo mật và khả năng sử dụng.',
    'Benefits we offer': 'Lợi ích chúng tôi cung cấp',
    'Unlock the full potential of the product with top-tier features and quality.':
        'Mở khóa toàn bộ tiềm năng của sản phẩm với các tính năng tuyệt vời và chất lượng hàng đầu.',
    'Artificial intelligence, safe solution for beginners':
        'Trí tuệ nhân tạo, giải pháp an toàn cho người mới bắt đầu giao dịch',
    'More security and control over your money than the rest':
        'Bảo mật và kiểm soát tiền bạc nhiều hơn từ phần còn lại',
    'Flexible and easier mobile payments for all investors':
        'Thanh toán di động linh hoạt và dễ dàng hơn cho tất cả các nhà đầu tư',
    'Earn daily interest. When storing cryptocurrency at AZChain':
        'Kiếm lãi hàng ngày. Khi lưu trữ tiền điện tử tại AZChain',
    '$AZC is the native utility token of the AZChain ecosystem, designed to facilitate secure and efficient transactions within our platform. It plays a central role in maintaining network operations, incentivizing participation, and enabling access to tokenized real-world assets.':
        '$AZC là token tiện ích gốc của hệ sinh thái AZChain, được thiết kế để tạo điều kiện cho các giao dịch an toàn và hiệu quả trong nền tảng của chúng tôi. Nó đóng vai trò trung tâm trong việc duy trì hoạt động của mạng lưới, khuyến khích sự tham gia và cho phép truy cập vào các tài sản thực tế được mã hóa.',
    'Token name': 'Tên mã thông báo',
    'Token symbol': 'Biểu tượng Token',
    Network: 'Mạng',
    'Total supply': 'Tổng cung',
    'Decimal code': 'Mã số thập phân',
    'Token type': 'Loại mã thông báo',
    Practicality: 'Tính thiết thực',
    'Smart contract': 'Hợp đồng thông minh',
    'All AZChain smart contracts will be audited by top global security firms such as Certik, Cyberscope, Quantstamp, Salus before launch.':
        'Tất cả Hợp đồng thông minh AZChain sẽ được kiểm toán bởi các công ty bảo mật hàng đầu thế giới như Certik, Cyberscope, Quantstamp, Salus trước khi phát hành.',
    'Unleash the power of tokenizing real-world assets and digitization with the AZChain ecosystem':
        'Giải Phóng Sức Mạnh Của Việc Token Hóa Tài Sản Thực và chuyển đổi số Với Hệ Sinh Thái AZChain',
    'The AZChain ecosystem is designed to seamlessly connect blockchain technology with real-world assets, providing an innovative way to invest in industries like real estate, agriculture, and digitization by bridging businesses and consumers worldwide. By tokenizing these assets, AZChain turns them into digital assets that can be easily bought, sold, and traded, opening up new investment opportunities for everyone.':
        'Hệ sinh thái AZChain được thiết kế để kết nối liền mạch công nghệ blockchain với các tài sản thực tế, mang đến một cách đầu tư sáng tạo vào các ngành như bất động sản, nông nghiệp và chuyển đổi số, kết nối doanh nghiệp và người tiêu dùng trên khắp thế giới. Bằng cách token hóa các tài sản này, AZChain biến chúng thành tài sản kỹ thuật số có thể được mua, bán và giao dịch một cách dễ dàng, mở ra cơ hội đầu tư mới cho mọi người.',
    'E-commerce': 'THƯƠNG MẠI ĐIỆN TỬ',
    Exchange: 'SÀN GIAO DỊCH',
    'NFT MARKETPLACE': 'SÀN GIAO DỊCH NFT',
    Development: 'Phát triển',
    Research: 'Nghiên cứu',
    'Idea generation': 'Lên ý tưởng',
    'Implementation planning': 'Lập kế hoạch triển khai',
    'Team building': 'Xây dựng đội ngũ',
    'Platform building': 'Xây dựng các nền tảng',
    'Community connection': 'Kết nối cộng đồng',
    'Developing private chains': 'Phát triển Chain riêng',
    'Q4/2024: Launch of the AZChain Project':
        'Quý IV/2024: Ra mắt dự án AZChain',
    'Release of NFTs and Tokens': 'Phát hành NFT và Token',
    'Quarter I: Deploy the Carbon project': 'Quý I: Triển khai dự án Carbon',
    'Quarter III: Launch the decentralized Carbon exchange':
        'Quý III: Ra mắt sàn giao dịch Carbon phi tập trung',
    'Quarter IV: Deploy the decentralized e-commerce project':
        'Quý IV: Triển khai dự án TMĐT phi tập trung',
    'Quarter I: Launch decentralized e-commerce':
        'Quý I: Ra mắt TMĐT phi tập trung',
    'Quarter II - Quarter III: Develop and expand':
        'Quý II - Quý III: Phát triển và Mở rộng',
    'Develop a national and Southeast Asian retail chain':
        'Phát triển chuỗi bán lẻ toàn quốc và Đông Nam Á',
    'Quarter IV: 2026 Listing token': 'Quý IV: 2026 Listing token',
    'Expand and develop the ecosystem': 'Mở rộng và Phát triển hệ sinh thái',
    'IPO in Vietnam and Singapore': 'IPO Việt Nam và Singapore',
    Partners: 'Đối tác',
    'SMART CONTRACT': 'HỢP ĐỒNG THÔNG MINH',
    'International and will be implemented worldwide':
        'Mang tính quốc tế và sẽ được triển khai trên toàn thế giới',
    'Disclaimer: Cryptocurrency may be unregulated in your jurisdiction. The value of cryptocurrencies may go down as well as up. Profits may be subject to capital gains or other taxes applicable in your jurisdiction.':
        'Khuyến cáo: Tiền điện tử có thể không được quản lý trong khu vực của bạn. Giá trị của tiền điện tử có thể giảm cũng như tăng. Lợi nhuận có thể chịu thuế lãi vốn hoặc các loại thuế khác áp dụng tại khu vực của bạn.',
    'Buy now': 'Mua ngay',
    FAQ: 'Câu hỏi thường gặp',
    Media: 'Truyền thông',
    Disclaimers: 'Khuyến cáo'
}

export default translation
